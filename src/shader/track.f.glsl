...
float inShadow() {
    ...
    for (int x = -1; x <= 1; x++) {
        for(int y = -1; y <= 1; y++) {
            ...
            shadow += curDepth > pcfDepth ? 1.0 : 0.0; // no need for bias for track   
        }    
    }
    shadow /= 9.0;
    return shadow;
...
void main() {
    ...
    vec3 tex = vec3(0.4, 0.4, 0.4);
    if (abs(TexCoord.x - 0.08) <= 0.02 || abs(TexCoord.x - 0.92) <= 0.02) 
        tex = vec3(1.0, 1.0, 1.0);
    vec3 amb = 0.4 * tex;
    vec3 diff = 0.6 * tex * max(dot(wi, n), 0);
    fragColor = vec4(amb + diff * (1 - inShadow()), 1);
}
